...
import { EmojiPickers } from "../UI/EmojiPicker.tsx";
...
export const MessageInput = ({ onSend }: Props) => {
    ...
    useEffect(() => {
        ...
        const handleClickOutside = (event: MouseEvent) => {
            ...
            if (emojiAreaRef.current && !emojiAreaRef.current.contains(target)) {
    ...
    return (
        <form onSubmit={handleSubmit} className="flex w-full items-center">
            ...
            <div className="relative px-2" ref={emojiAreaRef}>
                {showEmojiPicker && (
                    <div className="absolute bottom-12 right-0 z-10">
                        <EmojiPickers onEmojiSelect={handleEmojiSelect} />
                    </div>
                )}
                <button
                    type="button"
